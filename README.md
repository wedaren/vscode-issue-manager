...
| 配置项 | 类型 | 默认值 | 描述 |
|--------|------|--------|------|
| `issueManager.search.fulltext.caseSensitive` | 布尔值 | `false` | 全文搜索时是否区分大小写 |
| `issueManager.search.fulltext.useRegex` | 布尔值 | `false` | 全文搜索时是否启用正则表达式模式 |
| `issueManager.search.fulltext.wholeWord` | 布尔值 | `false` | 全文搜索时是否进行全词匹配 |
| `issueManager.search.fulltext.maxResults` | 数字 | `100` | 全文搜索返回的最大结果数量（1-500） |
| `issueManager.search.fulltext.maxSnippetsPerFile` | 数字 | `3` | 每个文件最多显示的匹配片段数量（1-10） |
| `issueManager.search.fulltext.contextLines` | 数字 | `0` | 匹配片段显示的上下文行数（0-5），0表示只显示匹配行 |
...
#### 🔍 全文内容搜索（新增）
- **完整内容扫描**：在问题的完整 Markdown 内容中搜索，不仅限于标题和摘要
- **多种搜索模式**：支持普通文本、正则表达式、全词匹配、大小写敏感等选项
- **智能结果预览**：直接显示匹配内容的上下文和行号
- **一键精确定位**：点击结果自动打开文件并跳转到匹配行
- **灵活配置**：通过设置自定义搜索行为（最大结果数、每文件片段数、上下文行数等）
...
详细说明请参见 [全文搜索功能文档](FULLTEXT_SEARCH.md)。
