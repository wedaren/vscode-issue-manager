...
**核心用户故事**: “作为一名用户，我的任务列表可能很庞杂，我希望能标记出当前最重要的几个问题，并在一个专门的视图中追踪它们。这个视图应该只包含我关注的问题，但同时也要展示它们完整的上下文（即所有的父级和子级），以便我能理解它们的全貌。同时，我希望在关注视图中也能像总览视图一样进行结构操作，包括创建问题、解除关联等。”

**验收场景 (Acceptance Scenarios)**:

- [ ] **场景1：添加关注** - 在 `问题总览` 视图中对一个节点使用“添加到关注”后，`关注问题` 视图会刷新，并展示该节点及其完整的上下文（所有祖先和所有后代）。
- [ ] **场景2：移除关注** - 在 `关注问题` 视图中对一个节点使用“移除关注”后，如果该节点及其后代均无其他关注点，它会从视图中消失。其祖先节点如果仅因为此节点而显示，也会一并消失。**注意：只有被直接关注的一级节点才可移除关注。**
- [ ] **场景3：结构同步** - 在 `关注问题` 视图中对节点进行任何结构调整（如拖拽、创建问题、解除关联），`问题总览` 视图会实时反映出完全相同的结构变化。
- [ ] **场景4：状态持久化** - 关闭并重新打开 VS Code 后，`关注问题` 视图中展示的内容保持不变。
- [ ] **场景5：结构操作** - 在 `关注问题` 视图中，可以通过右键菜单创建新的子问题，也可以对节点进行解除关联操作，行为与“问题总览”一致。
