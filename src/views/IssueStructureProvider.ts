 ...
 */
export class IssueStructureProvider
    implements vscode.TreeDataProvider<IssueStructureNode>, vscode.Disposable
{
    private _onDidChangeTreeData: vscode.EventEmitter<
        IssueStructureNode | undefined | null | void
    > = new vscode.EventEmitter<IssueStructureNode | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<IssueStructureNode | undefined | null | void> =
        ...
        try {
            ...
            if (!frontmatter || !frontmatter.issue_root_file) {
                return changeType === "create"
                    ? false
                    : await this.isFileRelatedToCurrent(fileName);
            }
     ...
     */
    private async syncFrontmatterRelations(
        fileName: string,
        ...
        try {
            ...
            if (frontmatter.issue_parent_file) {
                ...
                const success = await FrontmatterService.addChildToParent(
                    fileName,
                    frontmatter.issue_parent_file
                );
                ...
                const addChildSuccess = await FrontmatterService.addChildToParent(
                    fileName,
                    currentActiveFileName
                );
                const setParentSuccess = await FrontmatterService.setParentFile(
                    fileName,
                    currentActiveFileName
                );

                if (
                    (addChildSuccess || setParentSuccess) &&
                    this.nodeCache.has(currentActiveFileName)
                ) {
    ...
    private async isFileRelatedToCurrent(fileName: string): Promise<boolean> {
        ...
        try {
            ...
            if (frontmatter) {
                ...
                return (
                    frontmatter.issue_root_file === this.currentActiveFrontmatter.issue_root_file
                );
            }
            ...
            for (const existingFileName of filesToCheck) {
                ...
                try {
                    ...
                    if (childrenFiles.includes(deletedFileName)) {
                        const success = await FrontmatterService.removeChildFromParent(
                            deletedFileName,
                            existingFileName
                        );
                        if (success) {
    ...
    private async buildNodeRecursively(
        ...
        try {
            ...
            for (const childFileName of childrenFiles) {
                const childNode = await this.buildNodeRecursively(
                    childFileName,
                    visited,
                    nodeCache,
                    sessionCache
