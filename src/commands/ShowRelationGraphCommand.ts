...
export class ShowRelationGraphCommand {
    ...
    public async execute(uri?: vscode.Uri): Promise<void> {
        try {
            Logger.getInstance().info('[ShowRelationGraph] 命令开始执行');
            // 获取当前文件路径
            const filePath = uri?.fsPath || vscode.window.activeTextEditor?.document.uri.fsPath;
            Logger.getInstance().info('[ShowRelationGraph] 文件路径:', filePath);
            ...
            this.webviewManager.onMessage(panel, (message) => {
                Logger.getInstance().info('[ShowRelationGraph] 收到消息:', message.type);
                switch (message.type) {
                    case 'READY':
                        // Webview 就绪后发送数据
    ...
    private async sendGraphData(panel: vscode.WebviewPanel, filePath: string): Promise<void> {
        try {
            Logger.getInstance().info('[ShowRelationGraph] 开始获取图数据');
            // 从 GraphDataService 获取图数据
            const graphData: G6GraphData = await this.graphDataService.getGraphData(filePath);
            Logger.getInstance().info(`[ShowRelationGraph] 获取到图数据: 节点数=${graphData.nodes.length}, 边数=${graphData.edges.length}`);
