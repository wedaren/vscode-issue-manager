...
export async function executeCreateIssueFromCompletion(...args: unknown[]): Promise<void> {
    try {
        const { parentId, titleArg, background, insertMode, hasTrigger } =
            parseCreateIssueArgs(args);
        const editor = vscode.window.activeTextEditor;

        let title = titleArg && titleArg.trim().length > 0 ? titleArg.trim() : undefined;
        if (!title) {
            title = await vscode.window.showInputBox({
                prompt: "输入要创建的问题标题（取消将中止）",
            });
            if (!title) {
                return;
            }
...
export function parseCreateIssueArgs(args: unknown[]): {
...
async function insertLinkIntoEditor(
    ...
    const cursorIsAtLineEnd =
        previousSelection.isEmpty && previousSelection.active.isEqual(line.range.end);
