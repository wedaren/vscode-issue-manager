...
export function getAllChatRoles(): ChatRoleInfo[] {
    ...
    for (const md of all) {
        ...
        roles.push({
            ...
            timerToolTimeout: toFiniteNumber(fm.timer_tool_timeout),
            ...
            skills: Array.isArray(fm.skills) ? (fm.skills as unknown[]).map(String) : undefined,
...
export async function createChatRole(
    ...
    const fm: Partial<FrontmatterData> & ChatRoleFrontmatter = {
        ...
        timer_timeout: 60000,
        timer_tool_timeout: 60000,
        timer_max_execution: 600000,
        timer_max_retries: 3,
...
/**
 * 读取角色记忆文件正文，用于注入 system prompt。
 * 与 readAutoMemoryForInjection 不同：这是 LLM 主动写入的记忆，而非系统自动提取。
 * 文件不存在时返回空字符串（不自动创建，避免注入阶段产生副作用）。
 */
export async function readRoleMemoryForInjection(roleId: string): Promise<string> {
    const all = getIssueMarkdownsByType('role_memory');
    const md = all.find(m => m.frontmatter?.role_memory_owner_id === roleId);
    if (!md) { return ''; }
    try {
        const raw = Buffer.from(await vscode.workspace.fs.readFile(md.uri)).toString('utf8');
        const { body } = extractFrontmatterAndBody(raw);
...
/** 本地粗估 token 数（无网络依赖） */
function estimateTokensLocal(messages: vscode.LanguageModelChatMessage[]): number {
...
/**
 * 估算消息列表的 token 数。
 * 优先使用 VS Code LanguageModelChat.countTokens()（带 5s 超时），不可用或超时时按字符数粗估。
 */
export async function estimateTokens(messages: vscode.LanguageModelChatMessage[]): Promise<number> {
    try {
        ...
        // countTokens 不可用或超时，回落到字符估算
    }
    return estimateTokensLocal(messages);
}
...
/**
 * 获取或创建某成员在某协调者对话下的专属对话文件。
 * 通过 chat_role_id + chat_group_id 双 key 定位，跨 session 持久化。
 */
export async function getOrCreateGroupMemberConversation(
    coordinatorConvId: string,
    memberId: string,
    memberName: string,
    ...
    const all = getIssueMarkdownsByType('chat_conversation');
    for (const md of all) {
        if (!md.frontmatter?.chat_conversation) { continue; }
        const fm = md.frontmatter as unknown as ChatConversationFrontmatter & FrontmatterData;
        if (fm.chat_role_id === memberId && fm.chat_group_id === coordinatorConvId) {
            return md.uri;
        }
    }

    const role = getChatRoleById(memberId);
    const defaultModelFamily = role?.modelFamily
        || vscode.workspace.getConfiguration('issueManager').get<string>('llm.modelFamily')
        || 'gpt-5-mini';
    const title = `${memberName} @ ${coordinatorName}`;
    const fm: Partial<FrontmatterData> & ChatConversationFrontmatter = {
        chat_conversation: true,
        chat_role_id: memberId,
    ...
    const body = `# ${title}\n`;
    const uri = await createIssueMarkdown({ frontmatter: fm, markdownBody: body });
    if (!uri) { throw new Error(`无法创建成员对话文件：${title}`); }
    return uri;
}

/**
 * 从协调者对话文件中解析消息（支持 ## Assistant:RoleName 格式，自动过滤末尾状态标记）
 */
...
/** 从 markdown body 解析协调者对话消息列表（支持 ## Assistant:RoleName 格式） */
...
export async function getRecentConversationEntries(limit = 20): Promise<RecentConversationEntry[]> {
    ...
    // 协调者角色的成员文件（chat_group_id 关联）也归入对应角色名下
    for (const role of getAllChatRoles()) {
        for (const c of getConversationsForRole(role.id)) {
...
export async function deleteChatRole(roleId: string): Promise<DeleteRoleResult> {
    ...
    try {
        ...
        for (const c of convos) {
            ...
            // 删除对话关联的计划文件（若存在）
            if (c.planId && dir) {
                const planUri = vscode.Uri.file(path.join(dir, `${c.planId}.md`));
                try {
                    await vscode.workspace.fs.delete(planUri);
                    deleted++;
                } catch (e) {
                    errors.push(`删除计划 ${planUri.fsPath} 失败: ${e instanceof Error ? e.message : String(e)}`);
                }
            }
        ...
        // 删除角色的记忆文件（LLM 主动写入的 role_memory）
        const memories = getIssueMarkdownsByType('role_memory');
        for (const md of memories) {
            if ((md.frontmatter as Record<string, unknown>)?.role_memory_owner_id === roleId) {
                try {
                    await vscode.workspace.fs.delete(md.uri);
                    deleted++;
                } catch (e) {
                    errors.push(`删除角色记忆 ${md.uri.fsPath} 失败: ${e instanceof Error ? e.message : String(e)}`);
                }
            }
        }

        // 删除角色的自动提取记忆文件（系统维护的 role_auto_memory）
...
async function appendRawToLog(logUri: vscode.Uri, text: string): Promise<void> {
    ...
    // 队列清理：链完成后如果仍是自己，就删掉引用
    next.then(() => { if (logWriteQueues.get(key) === next) { logWriteQueues.delete(key); } });
    return next;
}
