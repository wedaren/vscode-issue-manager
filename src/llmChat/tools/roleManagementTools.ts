...
function executeListAvailableTools(): ToolCallResult {
    ...
    const templateSection = [
        ...
        '',
        '**个人助理**（中枢调度）: contextStrategy=generous, toolSets=[planning, terminal, browsing, knowledge_base, role_management]',
        '- 全量上下文 + 计划 + 终端 + 浏览器 + 知识库 + 角色管理，适合作为用户的全能入口',
        '',
        '**思维伙伴**（深度对话）: contextStrategy=generous, toolSets=[knowledge_base]',
        '- 全量上下文 + 知识库，不需要工具执行能力，专注思维碰撞',
        '',
        '**深度研究员**（系统研究）: contextStrategy=focused, contextSources=[memory, plan, linked_files, datetime], toolSets=[planning, knowledge_base, browsing]',
        '- 精选上下文避免干扰 + 计划管理持久化研究进度 + 浏览器查资料',
        '',
        '**长篇创作**（自主写作）: contextStrategy=minimal, toolSets=[planning, knowledge_base], autonomous=true, timerEnabled=true',
