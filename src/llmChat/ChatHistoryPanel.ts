...
import { parseConversationMessages, parseGroupConversationMessages } from './llmChatDataManager';
import type { ChatMessage, ChatRoleInfo, ChatGroupInfo, ChatGroupMessage } from './types';
...
/** 面板上下文：单聊或群聊 */
type PanelContext =
    | { type: 'role'; role: ChatRoleInfo }
    | { type: 'group'; group: ChatGroupInfo; members: ChatRoleInfo[] };
...
export class ChatHistoryPanel {
    ...
    private constructor(
        ...
        this.panel = panel;
        this._context = ctx;
        this._panelKey = ctx.type === 'role' ? ctx.role.id : `group:${ctx.group.id}`;
    ...
    static async openOrShow(
        ...
        if (existing && !existing._disposed) {
            ...
            existing._context = { type: 'role', role };
        ...
        const instance = new ChatHistoryPanel(panel, { type: 'role', role }, conversationUri, extensionUri);
        ChatHistoryPanel.panels.set(key, instance);
        return instance;
    }

    /** 获取或创建面板（群聊） */
    static async openOrShowGroup(
        group: ChatGroupInfo,
        ...
        if (existing && !existing._disposed) {
            existing._conversationUri = conversationUri;
            existing._context = { type: 'group', group, members };
            existing.panel.reveal(vscode.ViewColumn.One);
            await existing.refreshMessages();
            return existing;
        }
        ...
        const instance = new ChatHistoryPanel(
            panel,
    ...
    async refreshMessages(): Promise<void> {
        if (this._context.type === 'group') {
            const messages = await parseGroupConversationMessages(this._conversationUri);
            this.panel.webview.html = this.getGroupHtml(messages);
        } else {
    ...
    private getHtml(messages: ChatMessage[]): string {
        if (this._context.type !== 'role') { return ''; }
        const nonce = getNonce();
        const webview = this.panel.webview;
        const roleName = this._context.role.name;
        const roleAvatar = this._context.role.avatar || 'hubot';
    ...
    private getGroupHtml(messages: ChatGroupMessage[]): string {
        if (this._context.type !== 'group') { return ''; }
        const nonce = getNonce();
        const webview = this.panel.webview;
        const groupName = this._context.group.name;
        const groupAvatar = this._context.group.avatar || 'organization';
        const membersMap = new Map(this._context.members.map(m => [m.name, m]));

        const messagesHtml = messages.map(m => {
            const isUser = m.role === 'user';
            const cls = isUser ? 'msg-user' : 'msg-assistant';
            const memberRole = !isUser && m.roleName ? membersMap.get(m.roleName) : undefined;
            const icon = isUser ? 'person' : (memberRole?.avatar || 'hubot');
            const label = isUser ? '我' : (m.roleName || groupName);
            const time = formatTime(m.timestamp);
            return `<div class="msg ${cls}">
                <div class="msg-header">
                    <span class="msg-icon codicon codicon-${icon}"></span>
        ...
        return this.wrapHtml(nonce, webview, groupName, groupAvatar, messagesHtml, messages.length === 0);
    }
    ...
    private wrapHtml(
        ...
        const emptyHtml = isEmpty
            ? `<div class="empty-state">
                ...
                <div>开始${this._context.type === 'group' ? '群组讨论' : '对话'}吧</div>
